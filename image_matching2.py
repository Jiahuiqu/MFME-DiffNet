...
parser.add_argument('--path_HSI', default="/media/xidian/4b9949b2-d832-4f6e-9211-60d8e6480133/data/PaviaC-PaviaU/test/LRHS/", type=str, help='input files')
parser.add_argument('--path_match', default="/media/xidian/4b9949b2-d832-4f6e-9211-60d8e6480133/data/PaviaC-PaviaU/unpaired/unpair_pavia.mat", type=str, help='match file.')
parser.add_argument('--log_path', default="./log_1", type=str, help='alpha')
parser.add_argument('--save_path', default="./match/train/2/{}_2.mat", type=str, help='save_path')
...
if __name__ == "__main__":
    ...
    for step, path_i in enumerate(all_files):
        ...
        max_ssim = 0
        k = 0
        for row in range(0, HSI_matching_pca.shape[0]-patch_size, 40):
            for col in range(0, HSI_matching_pca.shape[1]-patch_size, 40):
                # 筛选
                k+=1
                ...
                # 上采下采两张图
                # 匹配图像
                matching_patch_up_2 = cv2.resize(HSI_matching_ro, (matching_patch.shape[0] * 1, matching_patch.shape[1] * 1), interpolation=cv2.INTER_LINEAR)
                # 高光谱图像
                HSI_pca_upsample_2 = cv2.resize(HSI_pca, (HSI_pca.shape[0] * 4, HSI_pca.shape[1] * 4), interpolation=cv2.INTER_LINEAR)
        ...
        savemat('./match/test/2/'+str(step)+".mat", {'match_img': match_img, 'match_score': match_score})
